predict.SMTS <- function(newdata, modelPath){
	...
	for(j in 1:nrow(model$classInfo)){
		predicted_class_new[which(predicted_class == model$classInfo$ID[j])] = model$classInfo$x[j]
	}
	 
	return(list(classPred = predicted_class_new, probVals = predicted_prob))
