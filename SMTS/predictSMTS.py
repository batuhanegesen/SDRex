...
class PredictSMTS:
    def __init__(self):
        self.rf_metamodel = None
    def predict_SMTS(self, newdata: pd.DataFrame, model_dict_path: str):
        ...
        node_status = self.make_node_status(RFins)
        codenew = self.generate_codebook(node_status, new_terminal, nofnode, nnewobs)
    ...
    def generate_codebook(self, nodestatus, terminal, nofterminal, nofobservations):
    ...
    def make_node_status(self, forest: RandomForestClassifier):
        node_status = []
        SPLIT_VALUE = 1
        LEAF_VALUE = -1
        MAX_LENGTH = 0
